...
class ContractBundleTests(unittest.TestCase):
    def test_release_has_exactly_five_stable_asset_names(self) -> None:
        self.assertEqual(
            (
                "health_data_edge_cases-0.6.0-py3-none-any.whl",
                "health_data_edge_cases-0.6.0.tar.gz",
                "health-data-edge-cases-0.6.0-contracts.zip",
                "health-data-edge-cases-0.6.0-provenance.json",
                "SHA256SUMS",
            ),
            release_assets.release_asset_names("0.6.0"),
        )
    ...
    def test_bundle_is_deterministic_allowlisted_and_self_verifying(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with zipfile.ZipFile(first) as archive:
                prefix = (
                    "health-data-edge-cases-0.6.0-contracts/"
                )
